...
Crescendo is not a library. It is not a SaaS product. It is a **template repository** that ships as a portable orchestration layer for [Antigravity](https://github.com/google-deepmind/antigravity) (Google's agentic coding platform). You clone the template, drop in your project files, select a domain profile, and the AI does the rest.
...
└── conductor/                         # The orchestration brain
    ...
    ├── code_styleguides/              # Style rules per language
    ├── templates/                     # Setup templates (style guides, workflow)
    ├── bin/                           # 9 orchestration scripts
    ...
    │   ├── conductor-inspector        # Profile/config inspector
...
Crescendo operates within Antigravity 2.0's quota system. There is no per-token billing — all models are included in the user's plan. The quota system IS the circuit breaker.
...
"budget": {
  ...
  "_budget_note": "In quota_based mode (Antigravity 2.0), budget_limit_usd and circuit_breaker_token_limit are null — the platform quota IS the limit. Set mode to 'api_key' and provide numeric values only when using direct API keys."
}
```

- `mode: "quota_based"` (default) — Antigravity handles rate limiting via platform quota. `budget_limit_usd` and `circuit_breaker_token_limit` are `null` because there's no per-token billing — the quota IS the circuit breaker.
...
- **Hardcoded references removed** — IronGate content genericized, org name parameterized
- **Autonomous override block** — GEMINI.md now coexists with autonomous mode
...
Each packaged skill includes `_upstream_source` and `_upstream_version` in its YAML frontmatter, tracking where it was forked from:

| Skill | Upstream Source | Version |
|-------|----------------|--------|
...
2. **Core files** — GEMINI.md, justfile, workflow.md, schemas, etc.
...
> [!NOTE]
> This document is a living artifact. It is updated after each commit to reflect the current state of Crescendo. The template SHA at the top of this document identifies the exact version described.
