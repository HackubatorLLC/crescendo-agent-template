---
name: crescendo-init
description: "Initializes a new multi-agent crescendo project by cloning the crescendo-agent-template and guiding the user on dropping in project files."
...
This skill is designed to help novice and experienced users easily spin up a new orchestration-ready repository based on the `{ORG_NAME}/crescendo-agent-template`.

> **Note:** The default template repository is `crescendo-agent-template`. If you need a different template, the user can specify it.
...
```bash
gh repo create {ORG_NAME}/<project-name> --template {ORG_NAME}/crescendo-agent-template --private --clone
```
...
Present the list of profiles to the user using the **`ask_question`** tool. Each option should show the domain name and a one-line summary. Example option text:
